...
const AIOverlay = ({ isOpen, onClose, onListen, isListening }) => {
  ...
  useEffect(() => {
    ...
    if (isListening && recognizedText && !recognizedText.toLowerCase().includes('luna')) {
  ...
  const processCommand = async (text) => {
    ...
    const smartHomeRegex = /(turn|switch|put|set) (on|off) (the )?([\w\s]+) (in|at) (the )?([\w\s]+)/i;
    ...
    if (match) {
      ...
      try {
        ...
        const apiUrl = `http://nakprciotsystemslabs.local/${encodeURIComponent(room)}/${encodeURIComponent(device)}/${action}`;
      ...
      <div className={`ai-overlay-content animated-entrance`}>
        ...
        <div className="ai-circle-container enhanced-shadow">
          ...
          <div className="ai-core enhanced-core-glow">
            <div className={`ai-core-pulse ${isListening || isSpeaking ? 'pulse-active' : ''}`}></div>
          </div>
        ...
        <button 
          className={`ai-action-button enhanced-action ${isListening ? 'listening' : ''}`}
          onClick={onListen}
          ...
          <div className="ai-model-status">
            {isLoading ? (
              <>
                ...
                <p className="download-info">Please wait while the AI model downloads...</p>
              </>
            ) : (
              <>
                ...
                <p className="download-info">This will download approximately 250MB of data</p>
              </>
            )}
          </div>
        )}
